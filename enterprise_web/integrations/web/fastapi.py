...
def fastapi_register_route(
    router_instance, action_type, url_path, handler, get_repo_func, get_ident_func
):
    handler = partial(
        handler, identity=Depends(get_ident_func), repo=Depends(get_repo_func)
    )
