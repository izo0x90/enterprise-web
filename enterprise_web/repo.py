...
class EntityRepo(
    ABC, Generic[GenericEntity, GenericEntityId], metaclass=EntityRepoMeta
):
    ENTITY_CLS: type[GenericEntity]
    DATA_STORE_TYPE: Enum
    ...
    @abstractmethod
    def _by_ids(
        self, ids: Sequence[GenericEntityId], filters: Sequence[NEEDSTYPEHINT]
    ) -> Sequence[GenericEntity]:
    ...
    def get(
        self, ids: Sequence[GenericEntityId], filters: Sequence[NEEDSTYPEHINT]
    ) -> Mapping[GenericEntityId, GenericEntity]:
        entities = {}
        entities_to_retrive = []
...
class EntityRepoManager:
    ...
    def __getitem__(self, entity_name):
        ...
        if repo_type in self.repo_instances:
            ...
            session = self.session_getters_by_data_source_type[
                repo_type.DATA_STORE_TYPE
            ]()
            session.begin()
